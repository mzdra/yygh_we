...
export default {
  ...
  created() {
    ...
    this.hosScheduleId = this.$route.query.hosScheduleId;
    this.init();
  },
  ...
  methods: {
    ...
    getSchedule() {
      hospitalApi.getSchedule(this.hosScheduleId).then((response) => {
    ...
    submitOrder() {
      ...
      this.submitBnt = '正在提交'
      orderApi.sendOrders(this.schedule.hosScheduleId,this.patient.id)
        .then(response => {
          let orderId = response.data  // 请求得到订单编号
