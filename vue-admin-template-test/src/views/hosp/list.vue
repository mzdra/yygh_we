...
export default {
  ...
  methods: {
    fetchData(page = 1) {
      this.page = page;
      hosp.getPageList(this.page, this.limit, this.searchObj).then(response => {
    ...
    findAllProvince() {
      hosp.findByDictCode("Province").then(response => {
    ...
    provinceChanged() {
      ...
      hosp.findByParentId(this.searchObj.provinceCode).then(response => {
    ...
    updateStatus(id, status) {
      hosp.updateStatus(id, status).then(response => {
        this.fetchData(1);
      });
    }
  }
